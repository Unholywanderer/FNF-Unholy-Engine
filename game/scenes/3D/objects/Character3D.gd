class_name Character3D; extends AnimatedSprite3D;
...
var antialiasing:bool = true:
	get: return texture_filter == BaseMaterial3D.TEXTURE_FILTER_NEAREST
	#set(anti): texture_filter = Util.get_alias(anti)

var sing_anims:PackedStringArray = ['singLEFT', 'singDOWN', 'singUP', 'singRIGHT']

func _init(pos:Vector3 = Vector3.ZERO, char:String = 'bf', player:bool = false):
...
func load_char(new_char:String = 'bf') -> void:
	...
	icon = json.icon
	scale = Vector3(json.scale, json.scale, json.scale)
...
func copy(from:Character3D) -> void:
