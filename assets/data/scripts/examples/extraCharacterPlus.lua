if variant == '' and file_exists('songs/'.. song_root ..'/chart-pico.json') then
    -- same as extraChar example but this follows a chart
    local leJson = parse_json('songs/'..song_root..'/chart-pico') -- parse_json starts in 'assets/'
    ...
    Conductor.add_audio(3, 'pico/Voices-player', 0.7)
end